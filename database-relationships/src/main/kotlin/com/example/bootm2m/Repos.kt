...
data class Course(
    ...
    @ManyToMany(mappedBy = "enrolledIn") val studentsEnrolledIn: List<Student> = listOf()
...
data class ViewStudent(
    val id: Long, val name: String, val coursesEnrolledIn: Iterable<String>
)
...
class StudentsController(
    ...
    @GetMapping
    fun findAll(): Iterable<ViewStudent> = studentRepository.findAll().map { it.toView() }
    ...
    fun enroll(@PathVariable id: Long, @RequestBody enrollIn: EnrollInCourse): ViewStudent {
        ...
        val course =
            courseRepository.findById(enrollIn.courseId).orElseThrow { ResponseStatusException(HttpStatus.NOT_FOUND) }
...
data class ViewCourse(
    val id: Long, val name: String, val studentsEnrolled: Iterable<String>
)
...
class CourseController(
    ...
    @GetMapping
    fun findAll(): Iterable<ViewCourse> = courseRepository.findAll().map { it.toView() }
}
